...
test_autopilot_contract() {
  ...
  assert_contains "$S" "Autopilot heartbeat\. Run omega-mode show\. If it does not list autopilot: CronDelete this job and stop\. Otherwise re-invoke the run's execution skill on the next unfinished task per omega:autopilot phase 2; ask nothing\." \
    "the heartbeat prompt is fixed text and routes through the execution skill"
  assert_not_contains "$S" "Otherwise continue the run from the next unfinished task" \
    "the heartbeat never has the fresh turn execute tasks by hand"
  ...
  assert_contains "$S" "heartbeat starts it at the next :17 or :47" "the user is told an unstarted run starts itself"
  assert_contains "$S" "does not survive a restart" "a resumed session is told the heartbeat is session-only"
  # Every path that clears the mode disarms first: the pid lives on the line.
